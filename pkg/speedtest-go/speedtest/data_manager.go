...
func (td *TestDirection) StartContext(ctx context.Context, cancel context.CancelFunc, mainRequestHandlerIndex int) {
	if len(td.fns) == 0 {
	...
	timer := time.AfterFunc(td.manager.captureTime, td.closeFunc)

	// Cancellation monitor: listen for external context cancellation and stop cleanly
